...
for component_id = 1:max(component_id_list)
    ...
    for ii = 1:length(t_node_list)
        child1_track = child1_track + 1;
        tracks{component_id}{child1_track} = [];
        ...
        for jj = 1:length(path) - 1
            ...
            if link_flag(link_ind) == 0
                if isempty(tracks{component_id}{child1_track})
                    tracks{component_id}{child1_track} = [parent child];
                else
                    tracks{component_id}{child1_track}(end+1) = child;
                end
...
for ii = 1:length(detection_list)
    ...
    if ~isempty(movieInfo.kids{parent_id})     % case1: one child exists
        ...
        for jj = 1:length(div_candiList_old{ii})
            ...
            z_score = division_gap_test(movieInfo, zeros(data_size), child1, child2);
    ...
    if ~isnan(parent_id)
        ...
        for jj = 1:2
            ...
            if ~isnan(child_id)
                ...
                [parent_cost_min, parent_min] = min(parent_cost);
                if parent_candi(parent_min) ~= parent_id || parent_cost_min >= 33
