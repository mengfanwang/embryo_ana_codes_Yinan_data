...
for ii = 1:cell_num
    ...
    if isempty(ind_match)
        ...
        if cell_time ~= t
            ...
            % only the most match two can be child
            % find at most two kids with nearest cost
            [child_cost, child_loc] = sort(movieInfo.Cij{detection_id});
%             child_loc = child_loc(child_cost < Ci);
            if length(child_loc) > 2
                child_loc = child_loc(1:2);
            end
            child_candi = movieInfo.nei{detection_id}(child_loc);
            if ~isempty(movieInfo.kids{detection_id})
                child_candi(child_candi == movieInfo.kids{detection_id}) = [];
            end
            div_candi = div_candi(ismember(div_candi, child_candi));
            if isempty(div_candi)
                continue;
            end
...
%                 [~, parent_loc] = min(movieInfo.Cji{div_candi(jj)});
%                 if parent_loc == detection_id
...
[TP, TN, FP, gt_divPair, detect_divPair] = div_acc([], G, movieInfo, spot_table, data_size) ;
