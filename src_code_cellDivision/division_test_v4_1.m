...
load('detect_splitPair_0530.mat');
...
%% FP investigation
detect_fpPair = detect_allPair(detect_code_list~=1,:);
edge_ratio = zeros(size(detect_fpPair,1), 2);
for ii = 1:size(detect_fpPair,1)
    parent_id = detect_fpPair(ii,1);
    child1_id = detect_fpPair(ii,2);
    child2_id = detect_fpPair(ii,3);
    child_time = movieInfo.frames(child1_id);
    for jj = 1:2
        if jj == 1
            child_id = child1_id;
...
%         edge_ratio(ii,jj) = norm((movieInfo.orgCoord(child_id,:) ...
%             - movieInfo.orgCoord(parent_id,:)).*im_resolution);
        ...
        edge_ratio(ii,jj) = norm( (movieInfo.orgCoord(parent_id,:) + frame_shift -...
            movieInfo.orgCoord(child_id,:)) .* im_resolution);
    end
end
edge_ratio = max(edge_ratio,[],2)./min(edge_ratio,[],2); 
...
edge_ratio = nan(size(gt_fnPair,1), 2);
...
%                 edge_ratio(ii,jj) = norm((movieInfo.orgCoord(child_id,:) - movieInfo.orgCoord(parent_id,:)).*im_resolution);
                child_time = movieInfo.frames(child_id);
                frame_shift = getNonRigidDrift([0 0 0], movieInfo.orgCoord(child_id,:), ...
                    child_time-1, child_time, movieInfo.drift, movieInfo.driftInfo);
                edge_ratio(ii,jj) = norm( (movieInfo.orgCoord(parent_id,:) + frame_shift -...
                    movieInfo.orgCoord(child_id,:)) .* im_resolution);
