...
%         end
    ...
    if parent_time ~= t && child_small_flag
        ...
        div_candi = findNeighbor(movieInfo, parent_id);
...
for ii = 1:cell_num
    ...
    if ~detect_flag
        % two childs not seperated. try to find the second head
        if ~isempty(movieInfo.kids{parent_id})  && ...
                ~isempty(div_candiList_2nd{parent_id})
            % repeat the same step
            child1_id = movieInfo.kids{parent_id};
            child_time = movieInfo.frames(child1_id);
            gap_flag = false(length(div_candiList_2nd{parent_id}),1);
            dist2parent = inf(length(div_candiList_2nd{parent_id}),1);
            for jj = 1:length(div_candiList_2nd{parent_id})
                child2_id = div_candiList_2nd{parent_id}(jj);
                gap_flag(jj) = division_gap_test(movieInfo, zeros(data_size), child1_id, child2_id);
                frame_shift = getNonRigidDrift([0 0 0], [movieInfo.orgCoord(child1_id,:); ...
                    movieInfo.orgCoord(child2_id,:)], child_time-1, child_time, movieInfo.drift, movieInfo.driftInfo);
                dist2parent(jj) = norm(mean(movieInfo.vox{parent_id}) + frame_shift -...
            ...
            if gap_flag(minIdx)
                div_num = div_num + 1;
                detect_divPair(div_num,:) = [parent_id child1_id div_candiList_2nd{parent_id}(minIdx)];
                
                div_2ndnum = div_2ndnum + 1;
                detect_2ndPair(div_2ndnum, :) = [parent_id child1_id div_candiList_2nd{parent_id}(minIdx)];
            end
        end
    end
end
...
sum(over_merge_flag | any(too_large_flag, 2) | any(isnan(cell_match_flag),2) ...
    | any(not_head_flag,2) | any(too_short_flag,2) | any(not_neighbor_flag,2) ) 
