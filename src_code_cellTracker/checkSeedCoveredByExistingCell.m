function [yxz, new_p_k_vec, invalid_locs] = checkSeedCoveredByExistingCell(seed_region, ...
    p_k_vec, movieInfo, refine_res, embryo_vid, thresholdMaps, missed_frame, q)
...
% can_tl = length(movieInfo.tracks{movieInfo.particle2track(candidate_reg,1)});
% if ~isnan(p_k_vec(1))
%     seed_tl = length(movieInfo.tracks{movieInfo.particle2track(p_k_vec(1),1)});
% else
%     seed_tl = length(movieInfo.tracks{movieInfo.particle2track(p_k_vec(2),1)});
% end
% if seed_tl <= can_tl % we cannot remove a region in longer tracks to retrieve a lost region
%     return;
% end
...
if ~isnan(p_k_vec(1))
    ...
    [yxz, new_p_k_vec] = extractSeedsInGivenCell(candidate_reg, ...
        ...
        embryo_vid, thresholdMaps, parent_flag, q);
end
