function [movieInfo, detections_YXZ, voxIdxList] = tree2tracks_cell(...
...
%     disp(i);
    ...
    %invalid_cell = find(cellfun(@length, voxIdxList{i})<q.minSize);
    %voxIdxList{i}(invalid_cell) = {[]};
    %voxList{i}(invalid_cell) = {zeros(0,3)};
    %det_maps{i}(ismember(det_maps{i}, invalid_cell)) = 0;
