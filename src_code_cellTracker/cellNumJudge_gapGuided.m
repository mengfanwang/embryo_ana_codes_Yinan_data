...
% %% way 2.1 the node to be split itself is a stable node
% if size(movieInfo.arc_avg_mid_std,2) < 4
%     movieInfo.arc_avg_mid_std = cat(2, movieInfo.arc_avg_mid_std, ...
%         inf(size(movieInfo.arc_avg_mid_std,1),1));
% end
% if movieInfo.arc_avg_mid_std(left_cells{i}(1),4)==1
%     one_cell_flag = 1;
%     return;
% end
...
%% way 2.3: segmentation is not consistent in consecutive two frames
% flag = adjframesConsistency(left_cells{1}, movieInfo, ...
%         refine_res, g, o2m_flag);
% if ~flag 
%     % means these two are not consistent with followers, but merge them
%     % is a good choice
%     one_cell_flag = 1;
%     return;
% end
...
if true
    ...
    for i=1:min(max(numel(left_cnt)-1, numel(right_cnt)-1), max_num_frame_include)
        if i+1 <= length(left_cnt)
            ...
            % two new criterion
%             elseif sz_diff2test_region_left(i+1) > 0 && left_cnt(i+1) == 1
%                 mergeVSseg(1) = mergeVSseg(1) + 1;
%             elseif sz_diff2test_region_left(i+1) < 0 && left_cnt(i+1) > 1
%                 mergeVSseg(2) = mergeVSseg(2) + 1;
            end
        ...
        if i+1 <= length(right_cnt)
            ...
            % two new criterion
%             elseif sz_diff2test_region_right(i+1) > 0 && right_cnt(i+1) == 1
%                 mergeVSseg(1) = mergeVSseg(1) + 1;
%             elseif sz_diff2test_region_right(i+1) < 0 && right_cnt(i+1) > 1
%                 mergeVSseg(2) = mergeVSseg(2) + 1;
            end
...
if length(bases) == 2 % only consider one region linking to two regions
    ...
    %     if isinf(maxCost1) && isinf(maxCost2)
    %         one_cell_flag = 1;
    %     elseif isinf(maxCost1) || isinf(maxCost2)
    %         if min(maxCost1, maxCost2) < abs(g.observationCost)
    %             one_cell_flag = 0;
...
%% way 4:
% if the regions are adjacent all the time
% if ~(any(left_adjacent_flag == false) ||...
%         any(right_adjacent_flag == false))
%     one_cell_flag = 1;
%     return;
% end
