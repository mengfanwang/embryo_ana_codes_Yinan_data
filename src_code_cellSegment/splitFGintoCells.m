...
% val_id_loc = find(seedRegionIn,1);
% if ~isempty(val_id_loc)
%     reg1stId = seedRegionIn(val_id_loc);
% else
%     reg1stId = 0;
% end
...
comMaps.fmapComp = fg;
% generate the seed maps from existing foreground
seedsMap = seed_map_gen(fg, comMaps.eigPosMap, comMaps.eig2dPosMap);
% seedsMap = fg;
% seedsMap(comMaps.eigPosMap) = 0; % remove 3d principal curvature
% if ~isempty(find(seedsMap, 1))
%     % check if there is a seed totally masked by 3d principal curvature
%     seedsMap_foundBy2d = fg;
%     seedsMap_foundBy2d(comMaps.eig2dPosMap) = 0;
