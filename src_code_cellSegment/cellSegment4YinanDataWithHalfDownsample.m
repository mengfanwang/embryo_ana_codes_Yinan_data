...
mydir  = '/home/congchao/Desktop/wcc_embryo/yinan_data/crop_data_scale_0.5_temporal_aligned';
addpath('./');
% data folder
data_folder = mydir;
...
for i=1:numel(tif_files)
    ...
    % 3D version
    [zMap, synId, fMap] = Synquant4Embryo_Paramater(sm_im, q, 25, 1000, 0, 0.0001, 100);
...
parfor i=1:numel(tif_files)
    ...
    org_im = tifread(fullfile(tif_files(i).folder, tif_files(i).name));
    synId = id_mat{i};
    sigma = 1.5; % 2d principal curvature smooth scale is smaller
