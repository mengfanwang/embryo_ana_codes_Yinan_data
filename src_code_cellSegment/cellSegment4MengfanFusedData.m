...
mydir  = '/home/congchao/Desktop/wcc_embryo/yinan_data/mengfan_fused';
addpath('./');
% data folder
data_folder = mydir;
tif_files = dir(fullfile(data_folder, '/*.tif'));
% results folder
res_folder = fullfile(save_folder,'yinan_data_mengfan_fuse_res');
if ~exist(res_folder,'dir')
    mkdir(res_folder);
end
minIntensity = 90; % The middle of two Gaussian intensity distributions (
...
%% v6+1: repeat one more time to find missed cells--> this can repeat until no new cells exists, but if tracking
