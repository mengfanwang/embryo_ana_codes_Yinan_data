...
mydir  = fullfile(home_folder, 'Insync/ccwang@vt.edu/Google Drive/Projects/', ...
    'embyo_analysis/data/Mengfan_data_09102022/sameViewFusion/00242');
addpath('./');
% data folder
data_folder = mydir;
tif_files = dir(fullfile(data_folder, '/*_sample.tif'));
% results folder
res_folder = fullfile(data_folder,'res');
if ~isempty(tif_files) && ~exist(res_folder,'dir')
    mkdir(res_folder);
end
minIntensity = 300; % The middle of two Gaussian intensity distributions (
                    % manually set but should learn from data)
minSz = 50; % seed size (very loose to avoid losing seeds)
minCellSz = 200; % cell size (relatively tight, will use later)
maxSz = 1000;
scale_term = 1000;
%% synQuant
...
for i=1:numel(tif_files)
    ...
    % 3D version
    [zMap, synId, fMap] = Synquant4Embryo_Paramater(sm_im, q, minSz, maxSz, 0, 0.0001, 100);
...
for i=1:numel(tif_files)
    ...
    org_im = tifread(fullfile(tif_files(i).folder, tif_files(i).name));
    synId = id_mat{i};
    sigma = 1; % 2d principal curvature smooth scale is smaller
...
for i=1:numel(tif_files)
    ...
    %profile on;
    [newIdMap, thresholdMap] = regionWiseAnalysis4d_par_enabled(synId, ...
            eigAll,org_im, varMapAll, [], [], q);%, i
    %profile viewer;
    %profile off;
    toc;
    refine_res{i} = uint32(newIdMap);
    refine_res{i} = uint32(region_sanity_check_with_intensity(org_im, refine_res{i}, minCellSz, 255*minIntensity/scale_term));
    threshold_res{i} = uint8(thresholdMap);
    threshold_res{i}(refine_res{i}==0) = 0;
    if save_tif_file
        bnd_map = display_bnd_map_3d(org_im, refine_res{i});
        tifwrite(bnd_map, fullfile(res_folder, ['synQuant_refine_res_4d_v9_bnd_', tif_files(i).name]));
    end
end
...
%% remove regions based on rough intensity levels and size
% load(fullfile(res_folder, 'synQuant_refine_res_4d_v9.mat'), 'refine_res');
% for i=1:numel(tif_files)
%     refine_res{i} = region_sanity_check_with_intensity(org_im, refine_res{i}, minCellSz, 255*minIntensity/scale_term);
%     bnd_map = display_bnd_map_3d(org_im, regOut);
%     tifwrite(bnd_map, fullfile(res_folder, ['synQuant_refine_res_4d_v9_bnd4_', tif_files(i).name]));
% end
%% v6+1: repeat one more time to find missed cells
% this can repeat until no new cells exists, but if tracking step exists,
% no need to conduct this repeative step.
load(fullfile(res_folder, 'synQuant_refine_res_4d_v9.mat'), 'refine_res');
...
for i=1:numel(tif_files)
    ...
    [newIdMap, thresholdMap] = regionWiseAnalysis4d(synId, ...
            eigAll,org_im, varMapAll, test_ids, [], q);
    toc;
...
%% v6+1: repeat one more time to find missed cells--> this can repeat until no new cells exists, but if tracking, no need
load(fullfile(res_folder, 'synQuant_refine_res_4d_v9plus.mat'), 'refine_res');
id_mat_3rd = cell(numel(tif_files), 1);
for i=1:numel(tif_files)
    tic;
    fprintf('processing %d/%d file\n', i, numel(tif_files));
    org_im = tifread(fullfile(tif_files(i).folder, tif_files(i).name));
    ...
    posEigMap = eig_res_3d{i}>0;
    % 3D version
    id_mat_3rd{i} = Synquant4Embryo_2iter(sm_im, refine_res{i}, posEigMap);
    toc;
end
save(fullfile(res_folder, 'synQuant_res_3iter_indirect4v9.mat'), 'id_mat_3rd','-v7.3');

%% refine the new detected cells
load(fullfile(res_folder, 'synQuant_res_3iter_indirect4v9.mat'), 'id_mat_3rd');
load(fullfile(res_folder, 'synQuant_refine_res_4d_v9plus.mat'));
seg_res_folder = [];
id_mat_com = refine_res;
for i=1:numel(tif_files)
    cell_append = max(id_mat_3rd{i}(:));
    valid_fg = id_mat_com{i}>0;
    id_mat_com{i}(valid_fg) = id_mat_com{i}(valid_fg) + cell_append;
    id_mat_com{i} = id_mat_com{i} + id_mat_3rd{i};
end
refine_res_1st = refine_res;
threshold_res_1st = threshold_res;
...
for i=1:numel(tif_files)
    ...
    tic;
    test_ids = 1:double(max(id_mat_3rd{i}(:)));
%     [newIdMap, thresholdMap] = regionWiseAnalysis4d(synId, ...
%             eigAll,org_im, varMapAll, test_ids, [], q);
    [newIdMap, thresholdMap] = regionWiseAnalysis4d(synId, ...
                eigAll,org_im, varMapAll, test_ids, [], q);
    ...
    bnd_map = display_bnd_map_3d(org_im, refine_res{i});
        tifwrite(bnd_map, fullfile(res_folder, ['synQuant_refine_res_4d_v9_bnd_last_', tif_files(i).name]));
end
save(fullfile(res_folder, 'synQuant_refine_res_4d_v9plusplus.mat'), 'refine_res',...
    'threshold_res','-v7.3');
