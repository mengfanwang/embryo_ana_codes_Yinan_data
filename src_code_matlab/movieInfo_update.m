...
debug = false;
% movieInfo.Cji = cat(1, movieInfo.Cji, append_cells);
% movieInfo.preNei = cat(1, movieInfo.preNei, append_cells);
% movieInfo.preOvSize = cat(1, movieInfo.preOvSize, append_cells);
if ~isfield(movieInfo, 'drift')
    error('Drift not found!');
    movieInfo.drift = zeros(numel(refine_res),3);
end
...
if debug
    disp([movieInfo.nei{36834}, movieInfo.Cij{36834}]);
end
for i=1:numel(reg4upt)
    cur_reg = reg4upt(i);
    % upate vox based on voxIdx and drift
...
if debug
    z1 = cellfun(@length, movieInfo.nei);
    z2 = cellfun(@length, movieInfo.Cij);
    if ~isempty(find(z1-z2~=0,1))
        keyboard;
    end
end
% update preNei's nei
