...
%     fprintf('processing %d/%d tracks, id:%d\n', i, numel(movieInfo.tracks),i);
    ...
    if ~isempty(find(cur_track == 1997, 1))
        %keyboard;
    end
...
%     mr = cat(1, mergeReg{:});
%     sr = cat(1, sepReg{:});
%     if ~isempty(find(mr==4002,1)) || ~isempty(find(sr==4002,1))
%         keyboard;
%     end
    ...
    [refine_res, movieInfo, megReg] = mergedRegGrow(mergeReg, vidMap, ...
        refine_res, movieInfo, g.simple_merge, merge_error_prob, g.stableNodeTest);
