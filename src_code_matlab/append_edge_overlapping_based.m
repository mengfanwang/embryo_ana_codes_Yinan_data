...
for i=1:length(adj_nei)
    if post_flag % see post-neighbors' pre-overlapping region
        ...
        %[~, loc] = max(movieInfo.preOvSize{adj_nei(i)}(cur_locs));
        ...
        cur_locs = find(movieInfo.frames(neis)==cur_fr);
        %[~, loc] = max(movieInfo.ovSize{adj_nei(i)}(cur_locs));
...
for j=1:length(uni_fr)
    ...
    if length(nei_thisframe)>1
        % criteria: two regions'
        % largest overlapping neighbor (equal to the second one)
        %nei_thisframe_sz = cellfun(@length, ...
        %    movieInfo.voxIdx(nei_thisframe));
        
        % way1: largest ov ratio; way2: largest ov size
        %nei_thisframe_ov = adj_ov(nei_this_locs)./nei_thisframe_sz;

        %nei_thisframe_ov(~max_ov_node(nei_this_locs)) = -1;
        %[~, max2_idx] = maxk(nei_thisframe_ov,2);
        combinations = nchoosek((1:length(nei_thisframe)), 2);
        max2_idx = combinations(1,:);
        newOvDistance = inf;
        for cc = 1:size(combinations, 1)
            tmp_idx = combinations(cc,:);
            %flag = regionAdjacent(movieInfo, refine_res, tmp_idx(1), ...
            %   tmp_idx(2));
            %if flag % the two kids should be adjacent
            tmp_Vox = cat(1, movieInfo.vox{nei_thisframe(tmp_idx)});
%             frame_shift = movieInfo.drift(uni_fr(j),:) - ...
%                 movieInfo.drift(cur_fr,:);
        ...
        if post_flag
            ...
            %min_cost = min(movieInfo.Cij{cur_node}(nei_this_locs(max2_idx)));
            min_cost = min(movieInfo.Cij{cur_node});
        else
            oldCosts = movieInfo.Cji{cur_node}(nei_this_locs(max2_idx));
            %min_cost = min(movieInfo.Cji{cur_node}(nei_this_locs(max2_idx)));
        ...
        if newCost < (min_cost-0.001) && min_cost > cost_good2go
            ...
            if newCost < min(g.c_ex,g.c_en) % also need to < enter/exit cost
                ...
                %                 min_cost = min_cost / 2 + 1e-5*rand;
                %                 if limit_flag
                %                     edge_cost = max(min_cost, newCost - oldCost);
                %                 else
                %                     edge_cost = newCost - oldCost;
                %                 end
                %return;
            end
        end
    ...
    % should we consider that this feasible node is only feasible to
    % current node? we should be, but current setting does cause clear
    % error, so we do not add it now
