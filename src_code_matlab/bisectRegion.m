...
%     base_intensities = sort(vidMap{base_frame}(base_voxIdx{j}),'ascend');
%     lb = base_intensities(round(0.05*length(base_intensities)));
%     ub = base_intensities(round(0.95*length(base_intensities)));
%     inter_idx = intersect(base_voxIdx{j}, root_voxIdx);
%     inter_vals = vidMap{root_frame}(inter_idx);
%     base_centers{j} = inter_idx(inter_vals>lb && inter_vals<ub);
...
%fMap = false(size(idComp));%imfill(idComp==real_id ,'holes');
% real_id = refine_res{root_frame}(root_voxIdx(1));
%union_idxs(idComp(union_idxs)~=real_id & idComp(union_idxs)>0) = [];
